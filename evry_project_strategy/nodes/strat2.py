...
def run_demo():
    ...
    #strategy 1
#    d_init = robot.getDistanceToFlag()
...
#        pathx_a,pathx_b,pathx_c,pathx_d = robot.get_polynomial_time_scaling_3rd_order(robot.x,2,-21.21320344,0,0.5)
#        pathy_a,pathy_b,pathy_c,pathy_d  = robot.get_polynomial_time_scaling_3rd_order(robot.y,2,21.21320344,0,0.5)
#        angle_robot = 0.15
...
#        angle_robot = 0
#    else :
#        pathx_a,pathx_b,pathx_c,pathx_d = robot.get_polynomial_time_scaling_3rd_order(robot.x,2,0,0,0.5)
#        pathy_a,pathy_b,pathy_c,pathy_d = robot.get_polynomial_time_scaling_3rd_order(robot.y,2,-30,0,0.5)
#        angle_robot = -0.12
#    t = 0
...
#        robot.set_speed_angle(velocity,angle)
    ...
    # stratigy 3 : 
    # # # Potential field with out obstacle # #

    if(robot.robot_name=='robot_1'):
        q_goal = np.array([-21.21320344,21.21320344])
    elif(robot.robot_name=='robot_2'):
        q_goal = np.array([21.21320344,21.21320344])
    elif(robot.robot_name=='robot_3'):
        q_goal = np.array([0,-30])
    ...
    PI = 3.141592654
    angleTolerance = 0.05
    q_rob1 =  np.array([21.21,-21.21])
    q_rob2 =  np.array([-21.21,-21.21])
    q_rob3 =  np.array([0,30])
    eta1 = 15
    eta2 = 15
    eta3 = 15
    headingError = [0.0,0.0]
 ...
 #       print(f"Distance sonor {robot.robot_name}:{robot.get_sonar()}")
        if(robot.robot_name=='robot_1'):
            q_rob1 =np.array([robot.x,robot.y])
        elif(robot.robot_name=='robot_2'):
            q_rob2 =np.array([robot.x,robot.y])
        elif(robot.robot_name=='robot_3'):
            q_rob3 =np.array([robot.x,robot.y])

#       if (sonar < 4.9 ): 
#           velocity = 0.1
#            angle = -0.5
#        else :
        q = np.array([robot.x,robot.y])
        error = np.subtract(q,q_goal)
        d = LA.norm(error)
#        print(f"Distance to flag {robot.robot_name}: {d}")
#        print(f" position {robot.robot_name}: {q[0]} , {q[1]}")
        if(distance > 5): 
            U = Ka*d
            F_attractive = -np.multiply(error,Ka/d)
        else : 
            U = 0.5*Ka*d**2
            F_attractive = -np.multiply(error,Ka)
        print(f" attractive potential field {robot.robot_name}: {U}")
        # repulsive potential field for robot 1
        if(robot.robot_name=='robot_1'):
            q_rob1 =np.array([robot.x,robot.y])
            error1 = np.subtract(q,q_rob2)
            error2 = np.subtract(q,q_rob3)
            D1 = LA.norm(error1)
            D2 = LA.norm(error2)
            if(D1 < eta2):
                F_repx1 =(Kr/(D1**2))*((1/D1)-(1/eta2))*error1[0]
                F_repy1 =(Kr/(D1**2))*((1/D1)-(1/eta2))*error1[1]
            else: 
                F_repx1 = 0
                F_repy1 = 0
            
            if (D2<eta3):
                F_repx2 =(Kr/(D2**2))*((1/D2)-(1/eta3))*error2[0]
                F_repy2 =(Kr/(D2**2))*((1/D2)-(1/eta3))*error2[1]
            else: 
                F_repx2 = 0
                F_repy2 = 0
#            print(f" position {robot.robot_name}: {q_rob1[0]} , {q_rob1[1]}")
        # repulsive potential field for robot 2
        elif(robot.robot_name=='robot_2'):
            q_rob2 =np.array([robot.x,robot.y])
            error1 = np.subtract(q,q_rob1)
            error2 = np.subtract(q,q_rob3)
            D1 = LA.norm(error1)
            D2 = LA.norm(error2)
            if(D1 < eta1):
                F_repx1 =(Kr/(D1**2))*((1/D1)-(1/eta1))*error1[0]
                F_repy1 =(Kr/(D1**2))*((1/D1)-(1/eta1))*error1[1]
            else: 
                F_repx1 = 0
                F_repy1 = 0
            ...
            if (D2<eta3):
                ...
                F_repx2 = 0
                F_repy2 = 0
            print(f" position {robot.robot_name}: {q_rob2[0]} , {q_rob2[1]}")
        # repulsive potential field for robot 3
        elif(robot.robot_name=='robot_3'):
            q_rob3 =np.array([robot.x,robot.y])
            error1 = np.subtract(q,q_rob1)
            error2 = np.subtract(q,q_rob2)
            D1 = LA.norm(error1)
            D2 = LA.norm(error2)
            if(D1 < eta1):
                F_repx1 =(Kr/(D1**2))*((1/D1)-(1/eta1))*error1[0]
            ...
            if (D2<eta2):
                F_repx2 =(Kr/(D2**2))*((1/D2)-(1/eta2))*error2[0]
                F_repy2 =(Kr/(D2**2))*((1/D2)-(1/eta2))*error2[1]
            else: 
                F_repx2 = 0
                F_repy2 = 0
            print(f" position {robot.robot_name}: {q_rob3[0]} , {q_rob3[1]}")
        
        
        dx = F_attractive[0] + F_repx1 + F_repx2
        dy = F_attractive[1] + F_repy1 + F_repy2
        velocity = sqrt((dx**2 ) + (dy**2))
        next_x = robot.x + step*dx
        next_y = robot.y + step*dy
       
        waypointHeading = atan2(next_y,next_x)
         # angle error 
        headingError[1] = - waypointHeading + robot.yaw  
        # derivative of the angle error
        d_headError = (headingError[1]-headingError[0])/step 

        # angle borders     
        if (headingError[1]> PI):
            headingError[1] = headingError[1] - (2 * PI)
        elif (headingError[1] < -PI):
            headingError[1] = headingError[1] + (2 * PI)
        print(f" headingError {robot.robot_name}: {headingError[1]}")        
        # PI controller
        if (abs(headingError[1])>angleTolerance):
            angle = Kp * headingError[1] + Ki*d_headError
        else:
            angle = 0
        print(f" angular velocity {robot.robot_name}: {angle}")
        robot.set_speed_angle(velocity,angle)
        #Finishing by publishing the desired speed. DO NOT TOUCH.

        headingError[0]=headingError[1]    
        rospy.sleep(0.5)
